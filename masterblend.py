...
class cinza18_add(bpy.types.Operator):
    ...
    def execute(self, context):        
        ...
        bpy.context.scene.view_settings.look = 'False Color - Punchy'
        ...
        for material in bpy.data.materials:          
            ...
            diffuse_gray18 = material.node_tree.nodes.new(type='ShaderNodeBsdfDiffuse')
            diffuse_gray18.name = 'diffuse_gray_18'    
            Output_gray18 = material.node_tree.nodes.new(type='ShaderNodeOutputMaterial')
            Output_gray18.name = 'Output_node_diffuse_gray_18'
            material.node_tree.links.new(diffuse_gray18.outputs['BSDF'], Output_gray18.inputs['Surface'])    
            diffuse_gray18.inputs[0].default_value = (0.18, 0.18, 0.18, 1)

            if material.node_tree and "Output_node_diffuse_gray_18" in material.node_tree.nodes:
                node_to_active = material.node_tree.nodes["Output_node_diffuse_gray_18"]
                material.node_tree.nodes.active = node_to_active
...
class cycles_ambiente_dia(bpy.types.Operator):
    bl_idname = "cycles_dia.ilum"
    bl_label = "cycles_ambient"
    import bpy
    def execute(self, context):
        bpy.data.worlds['World'].use_nodes = True
        ...
        dir = os.path.dirname(script_file)
        caminho77 = os.path.join(dir, 'hdri')
        caminho77 = "{}\\dia.exr".format(caminho77)
        env_node.image = bpy.data.images.load(caminho77)
        caminho77 =""
...
class cycles_ambiente_tarde(bpy.types.Operator):
    bl_idname = "cycles_tarde.ilum"
    bl_label = "cycles_ambient_tarde"
    import bpy
    def execute(self, context):
        bpy.data.worlds['World'].use_nodes = True
        ...
        dir = os.path.dirname(script_file)
        caminho77 = os.path.join(dir, 'hdri')
        caminho77 = "{}\\tarde.exr".format(caminho77)
        env_node.image = bpy.data.images.load(caminho77)
        caminho77 =""
